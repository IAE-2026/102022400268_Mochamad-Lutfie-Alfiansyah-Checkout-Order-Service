...
class CheckoutOrderApiTest extends TestCase
{
    ...
    public function test_api_key_is_required(): void
    {
        ...
        $this->getJson('/api/v1/unknown-resource')
            ->assertStatus(401)
    ...
    public function test_order_resource_can_be_created_directly_for_assignment_rubric(): void
    {
        $order = $this->postJson('/api/v1/orders', [
            'user_id' => 1,
            ...
            ->assertJsonPath('status', 'success')
            ->assertJsonPath('data.status', 'paid')
            ->assertJsonPath('data.items.0.product_id', 99)
            ->json('data');
    ...
    public function test_validation_errors_and_missing_resources_use_contract_wrapper(): void
    {
        ...
        $this->postJson('/api/v1/payment/methods', [], $this->headers())
            ->assertStatus(405)
            ->assertJson([
                'status' => 'error',
